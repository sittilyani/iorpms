...
$sql = "SELECT COUNT(*) as total
        FROM stock_taking
        WHERE MONTH(stock_take_date)=? AND YEAR(stock_take_year)=? $cond";
...
$sql = "SELECT COUNT(*) as total FROM stock_taking WHERE MONTH(stock_take_date) = ? AND YEAR(stock_take_year) = ? $all_conditions";
$stmt = $conn->prepare($sql);
...
// Total Stock Value for selected month ONLY
$sql = "SELECT SUM(physical_count * pack_price) as total_value FROM stock_taking WHERE MONTH(stock_take_date) = ? AND YEAR(stock_take_year) = ? $all_conditions";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $filter_month, $filter_year);
$stmt->execute();
$result = $stmt->get_result();
...
// Total Stock Value for selected month ONLY (same as above, but we'll use it for display)
$stats['total_value'] = $stats['current_month_value'];
...
// Expiring Soon (within 3 months from first day of selected month)
...
// Expiring 3-6 Months from first day of selected month
$sql = "SELECT SUM(physical_count * pack_price) as total_cost FROM stock_taking
...
if ($access_level === 'facility') {
    ...
    // Add filter conditions to facility count query
    if (!empty($filter_subcounty)) {
        $total_facilities_sql .= " AND f.subcountyname = '$filter_subcounty'";
    }
    if (!empty($filter_facility)) {
        $total_facilities_sql .= " AND f.facilityname = '$filter_facility'";
    }

    // Now find which of these MOH facilities reported in selected month
    $reporting_facilities_sql = "SELECT COUNT(DISTINCT s.facilityname) AS reporting_facilities
                                ...
                                AND YEAR(s.stock_take_year) = $filter_year
...
// Actual submissions (unique MOH facilityname + classification pair in the selected month)
...
$sql = "SELECT facilityname, COUNT(*) as count
        ...
        AND YEAR(stock_take_year) = $filter_year
        $all_conditions
...
$sql = "SELECT
        ...
        FROM stock_taking
        WHERE YEAR(stock_take_year) = '$filter_year'
        $all_conditions
...
<div class="stats-grid">
    ...
    <!-- Monthly Stock Value Comparison -->
    <div class="stat-card warning">
        ...
        <?php
        // Calculate previous month's value for comparison
        $prev_value_sql = "SELECT SUM(physical_count * pack_price) as total_value FROM stock_taking WHERE MONTH(stock_take_date) = ? AND YEAR(stock_take_year) = ? $all_conditions";
    ...
    <!-- Expiring Stock Cards (keep as is) -->
    ...
    <!-- Low Stock Items (filtered by month) -->
    <div class="stat-card info">
        <?php
        // Low Stock for selected month only
        $low_stock_sql = "SELECT COUNT(*) as low_stock FROM stock_taking
                        WHERE physical_count < 10
                        AND MONTH(stock_take_date) = $filter_month
                        AND YEAR(stock_take_year) = $filter_year
                        $all_conditions";
        $low_result = $conn->query($low_stock_sql);
        $monthly_low_stock = $low_result->fetch_assoc()['low_stock'] ?? 0;
        ?>
        <div class="stat-number"><?php echo number_format($monthly_low_stock); ?></div>
        <div class="stat-label">Low Stock Items (<?php echo getMonthName($filter_month); ?>)</div>
        <div class="stat-change warning">
            <i class="fa fa-exclamation-triangle me-1"></i>
            <?php echo $monthly_low_stock > 0 ? 'Needs urgent attention' : 'All good!'; ?>
        </div>
    </div>
</div>
    ...
    <div class="charts-grid">
        ...
        <div class="chart-card">
            <div class="chart-header">
                <h3><i class="fa fa-history"></i>Recent Stock Takes</h3>
