...
if (isset($_GET['p_id'])) {
    ...
    $stmt_last_visit->bind_param("s", $row_patient['mat_id']);
    ...
    if ($row_last_visit && !empty($row_last_visit['last_comp_date'])) {
        ...
        $last_comp_date = "No dispensing record in pharmacy"; // Assign a default message
...
function displayMissingDates($conn, $mat_id) {
    ...
    if (empty($missing_dates)) {
        ...
        echo "<div style='background-color: yellow; color: red; padding: 10px; margin-left: 20px; margin-bottom: 20px; line-height: 2; width: 50%; border: none;'>";
