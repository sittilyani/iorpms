...
$sql = "SELECT COUNT(DISTINCT T1.mat_id) as weaned_bupCount
        FROM patients T1
        INNER JOIN pharmacy T2 ON T1.mat_id = T2.mat_id
        WHERE T1.current_status = 'weaned'
        AND T1.sex = 'female'
        AND T2.drugName = 'methadone'
...
if ($stmt->execute()) {
    ...
    $weaned_bupCount = $row['weaned_bupCount'];

    // Output the count as plain text
    echo $weaned_bupCount;
