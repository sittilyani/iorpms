...
<body>
    <div class="content-main">
        ...
        $sql = "SELECT p.*,
                       (SELECT COUNT(*) FROM photos WHERE mat_id = p.mat_id) as photo_count,
                       (SELECT image FROM photos WHERE mat_id = p.mat_id ORDER BY visitDate DESC LIMIT 1) as latest_photo
                FROM patients p
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $hasPhoto = $row['photo_count'] > 0;
                $photoStatus = $hasPhoto ?
                    '<span class="photo-status photo-exists">Photo Exists</span>' :
                    '<span class="photo-status no-photo">No Photo</span>';
                ...
                if ($hasPhoto) {
                    ...
                    echo "<a href='photo_capture.php?p_id=" . $row['p_id'] . "&action=update' class='update-link'>Update Photo</a>";
                    echo "<a href='deletePhoto.php?p_id=" . $row['p_id'] . "' class='delete-link' onclick='return confirm(\"Are you sure you want to delete this photo?\")'>Delete Photo</a>";
                } else {
                    // If no photo exists, show Capture option
                    echo "<a href='photo_capture.php?p_id=" . $row['p_id'] . "'>Capture Photo</a>";
                }
