...
$sql = "SELECT pharmacy.mat_id, pharmacy.clientName, patient.sname, pharmacy.dosage, visitDate, cso, status, drugname
        FROM pharmacy
        INNER JOIN patient ON pharmacy.mat_id = patient.mat_id
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!in_array($row['mat_id'], $processed_mat_ids)) {
            ...
            echo "<tr><td>{$row['mat_id']}</td><td>{$row['clientName']}</td><td>{$row['sname']}</td><td>{$row['cso']}</td><td>{$row['status']}</td><td>{$row['drugname']}</td>";
