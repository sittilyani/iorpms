<?php
    // This file calculates: (buprenorphine8mgdisp * 3) - physical_count
    ...
    $total_dispensed_buprenorphine8mg = 0;

    // Get total buprenorphine8mg dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_buprenorphine8mg
            FROM pharmacy
            WHERE drugname LIKE '%buprenorphine8mg%'
    ...
    if ($result) {
        ...
        $total_dispensed_buprenorphine8mg = $row['total_dispensed_buprenorphine8mg'] !== null ? $row['total_dispensed_buprenorphine8mg'] : 0;
    }

    // Get physical count from GET parameter (if submitted)
    $physical_count = isset($_GET['physical_count_buprenorphine8mg']) ? floatval($_GET['physical_count_buprenorphine8mg']) : 0;

    // Calculate quantity for resupply: (dispensed * 3) - physical_count
    $quantity_resupply = ($total_dispensed_buprenorphine8mg * 3) - $physical_count;
