<?php
    // This file calculates: (naltrexoneimplantdisp * 3) - physical_count
    ...
    $total_dispensed_naltrexoneimplant = 0;

    // Get total naltrexoneimplant dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_naltrexoneimplant
            FROM pharmacy
            WHERE drugname LIKE '%naltrexoneimplant%'
    ...
    if ($result) {
        ...
        $total_dispensed_naltrexoneimplant = $row['total_dispensed_naltrexoneimplant'] !== null ? $row['total_dispensed_naltrexoneimplant'] : 0;
    }

    // Get physical count from GET parameter (if submitted)
    $physical_count = isset($_GET['physical_count_naltrexoneimplant']) ? floatval($_GET['physical_count_naltrexoneimplant']) : 0;

    // Calculate quantity for resupply: (dispensed * 3) - physical_count
    $quantity_resupply = ($total_dispensed_naltrexoneimplant * 3) - $physical_count;
