<?php
    // This file calculates: (naltrexone100mgdisp * 3) - physical_count
    ...
    $total_dispensed_naltrexone100mg = 0;

    // Get total naltrexone100mg dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_naltrexone100mg
            FROM pharmacy
            WHERE drugname LIKE '%naltrexone100mg%'
    ...
    if ($result) {
        ...
        $total_dispensed_naltrexone100mg = $row['total_dispensed_naltrexone100mg'] !== null ? $row['total_dispensed_naltrexone100mg'] : 0;
    }

    // Get physical count from GET parameter (if submitted)
    $physical_count = isset($_GET['physical_count_naltrexone100mg']) ? floatval($_GET['physical_count_naltrexone100mg']) : 0;

    // Calculate quantity for resupply: (dispensed * 3) - physical_count
    $quantity_resupply = ($total_dispensed_naltrexone100mg * 3) - $physical_count;
