<?php
    ...
    $total_dispensed_buprenorphine = 0;

    // Define the SQL query to sum total buprenorphine dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_buprenorphine
            FROM pharmacy
            WHERE drugname LIKE 'Buprenorphine 2mg'
    ...
    if ($result) {
        ...
        $total_dispensed_buprenorphine = $row['total_dispensed_buprenorphine'] !== null ? $row['total_dispensed_buprenorphine'] : 0;
        // Output the total (round to 2 decimal places)
        echo number_format($total_dispensed_buprenorphine, 2);
    } else {
