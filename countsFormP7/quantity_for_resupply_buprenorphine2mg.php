<?php
    // This file calculates: (buprenorphine2mgdisp * 3) - physical_count
    ...
    $total_dispensed_buprenorphine2mg = 0;

    // Get total buprenorphine2mg dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_buprenorphine2mg
            FROM pharmacy
            WHERE drugname LIKE '%buprenorphine2mg%'
    ...
    if ($result) {
        ...
        $total_dispensed_buprenorphine2mg = $row['total_dispensed_buprenorphine2mg'] !== null ? $row['total_dispensed_buprenorphine2mg'] : 0;
    }

    // Get physical count from GET parameter (if submitted)
    $physical_count = isset($_GET['physical_count_buprenorphine2mg']) ? floatval($_GET['physical_count_buprenorphine2mg']) : 0;

    // Calculate quantity for resupply: (dispensed * 3) - physical_count
    $quantity_resupply = ($total_dispensed_buprenorphine2mg * 3) - $physical_count;
