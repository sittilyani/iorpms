<?php
    ...
    $average_doses_Naltrexone = 0;

    // Define the SQL query to count active patients on Naltrexone within the last 6 days
    $query = "SELECT
        COUNT(DISTINCT mat_id) AS total_naltrex_count
    FROM pharmacy
    WHERE
    ...
    if ($result && $result->num_rows > 0) {
            ...
            $total_naltrex_count = $row['total_naltrex_count'];

            // Output the count
            echo $total_naltrex_count;
    } else {
            echo "0"; // If no active patientss on Naltrexone found within the last 6 days, display 0
    }
?>