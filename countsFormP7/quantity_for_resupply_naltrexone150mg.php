<?php
    // This file calculates: (naltrexone150mgdisp * 3) - physical_count
    ...
    $total_dispensed_naltrexone150mg = 0;

    // Get total naltrexone150mg dispensed
    $query = "SELECT SUM(dosage) AS total_dispensed_naltrexone150mg
            FROM pharmacy
            WHERE drugname LIKE '%naltrexone150mg%'
    ...
    if ($result) {
        ...
        $total_dispensed_naltrexone150mg = $row['total_dispensed_naltrexone150mg'] !== null ? $row['total_dispensed_naltrexone150mg'] : 0;
    }

    // Get physical count from GET parameter (if submitted)
    $physical_count = isset($_GET['physical_count_naltrexone150mg']) ? floatval($_GET['physical_count_naltrexone150mg']) : 0;

    // Calculate quantity for resupply: (dispensed * 3) - physical_count
    $quantity_resupply = ($total_dispensed_naltrexone150mg * 3) - $physical_count;
