...
// include '../includes/footer.php'; // Move footer to the end of the body
...
$result_drugs = $mysqli->query($sql_drugs);
if (!$result_drugs) {
    die("Error fetching drug data: " . $mysqli->error);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt) {
        ...
        error_log("Error preparing latest total qty query: " . $mysqli->error);
    }
    ...
    $stmt = $mysqli->prepare($sql_get_drug_id);
    if ($stmt) {
        $stmt->bind_param('s', $drugname);
        $stmt->execute();
        ...
        error_log("Error preparing drug ID query: " . $mysqli->error);
    }
    ...
    if ($drugID !== null) {
        ...
        if ($stmt) {
            ...
            error_log("Error preparing insert query: " . $mysqli->error);
        }
    } else {
