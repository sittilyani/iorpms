...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "INSERT INTO regimen (regimen_name) VALUES (?)";

        if ($stmt = $conn->prepare($sql)) {
                // Bind variables to the prepared statement as parameters
                $stmt->bind_param("s", $regimen_name);

                // Execute the prepared statement
                if ($stmt->execute()) {
                        // Success message and redirect
                        echo '<div style="color: green; background-color: #DAF7A6; height: 50px; padding: 15px; margin-left: 40px; margin-top: 30px; font-size: 18px;">Regimen added successfully</div>';
                        echo '<script>
                                        setTimeout(function(){
                                                window.location.href = "add_regimen.php";
                                        }, 3000);
                                    </script>';
                        exit();
                } else {
   ...
   <div class="main-content">
       <div class="form-group">
         ...
         <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <div class="form-group">
                <label for="regimen_name">Regimen:</label>  <br>
                <input type="text" id="regimen_name" name="regimen_name" required>
            </div>
