...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $cacheFileName = $cacheDir . md5($mat_id . $date_of_disp) . '.cache';
    if (file_exists($cacheFileName)) {
        ...
        if ($cacheData['count'] > 0) {
            ...
            header("Refresh: 5; URL=dispenze2.php");
            exit();
        }
    } else {
        ...
        file_put_contents($cacheFileName, serialize($cacheData));
        ...
        if ($stmt = $conn->prepare($sql)) {
                ...
                if ($stmt->execute()) {
                        ...
                        if ($result->num_rows == 1) {
                                ...
                                header("Refresh: 5; URL=dispense.php");
                                exit();
                        }
                } else {
...
if ($result->num_rows > 0) {
    ...
    $last_dispensing_date = date('d/m/Y', strtotime($row['date_of_disp'])); // Format the date
