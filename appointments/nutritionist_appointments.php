...
try {
    $sql = "SELECT lab_id, visitDate, mat_id, clientName, type_client, mode_drug_use, hiv_status, hbv_status, malaria_status, pregnancy_status, vdrl_status, urinalysis_status, hepc_status, lab_notes, lab_officer_name, next_appointment, date_created, appointment_status
            FROM laboratory
            WHERE next_appointment BETWEEN ? AND ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ss', $startDate, $endDate); // Removed $lab_id as it was undefined
    $stmt->execute();
...
<head>
    ...
    <title>Nutritionist Appointment Schedule</title>
...
<body>
    <div class="header">
        <div class="container">
            <h1 class="display-5 fw-bold"><i class="bi bi-calendar-check me-3"></i>Nutritionist Appointment Schedule</h1>
                ...
                <table class="table table-hover" id="appointmentsTable">
                    ...
                    <tbody>
                        <?php foreach ($appointments as $app): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($app['lab_id']); ?></td>
                                ...
                                <td><?php echo htmlspecialchars($app['type_client']); ?></td>
                                <td><?php echo htmlspecialchars($app['mode_drug_use']); ?></td>
                                <td><?php echo htmlspecialchars($app['pregnancy_status']); ?></td>
                                <td><?php echo htmlspecialchars($app['lab_notes']); ?></td>
                                <td><?php echo htmlspecialchars($app['lab_officer_name']); ?></td>
                                ...
                                <td class="action-buttons">
                                    <a href="update_laboratory_appointment.php?lab_id=<?php echo htmlspecialchars($app['lab_id']); ?>" class="btn btn-sm btn-outline-primary">
        ...
        function exportToExcel() {
            const exportData = appointments.map(app => {
                return {
                    ...
                    'Visit Type': app.type_client,
                    'Mode of Drug Use': app.mode_drug_use,
                    'Pregnancy Status': app.pregnancy_status,
                    'Lab Notes': app.lab_notes,
                    'Lab Officer Name': app.lab_officer_name,
