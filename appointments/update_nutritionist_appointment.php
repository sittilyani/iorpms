...
$sql = "SELECT p_id, mat_id, nutrition_tca, hcw_name FROM patients WHERE p_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $nutrition_tca_raw = trim($_POST['nutrition_tca']);
    $hcw_name            = trim($_POST['hcw_name']);

    // Normalize date (handles DATE or DATETIME)
    $nutrition_tca = date('Y-m-d', strtotime($nutrition_tca_raw));
    ...
    if (empty($errorMessage)) {
        try {
            ...
            $sql1 = "
                UPDATE patients
                SET nutrition_tca = ?, hcw_name = ?
                WHERE p_id = ?
            ";
            $stmt1 = $conn->prepare($sql1);
            $stmt1->bind_param('ssi', $nutrition_tca, $hcw_name, $p_id);
            $stmt1->execute();
            $stmt1->close();
            ...
            header('Location: nutritionist_appointments.php?success=1');
            exit;
...
$nutrition_tca = $currentSettings['nutrition_tca'] ?? '';
...
<head>
    <meta charset="UTF-8">
    <title>Update Nutritionist Appointment</title>
    <link href="../assets/css/bootstrap.css" rel="stylesheet">
</head>
<body>

<div class="container mt-5">
    <h3 class="mb-4">Update Nutritionist Appointment</h3>
    ...
    <form method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" name="nutrition_tca"
                   value="<?= htmlspecialchars(date('Y-m-d', strtotime($nutrition_tca))) ?>" required>
        </div>
        ...
        <a href="nutritionist_appointments.php" class="btn btn-secondary">Back</a>
    </form>
</div>
