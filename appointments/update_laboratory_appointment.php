...
$sql = "SELECT p_id, mat_id, laboratory_tca, hcw_name FROM patients WHERE p_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $laboratory_tca_raw = trim($_POST['laboratory_tca']);
    $hcw_name            = trim($_POST['hcw_name']);

    // Normalize date (handles DATE or DATETIME)
    $laboratory_tca = date('Y-m-d', strtotime($laboratory_tca_raw));
    ...
    if (empty($errorMessage)) {
        try {
            ...
            $sql1 = "
                UPDATE patients
                SET laboratory_tca = ?, hcw_name = ?
                WHERE p_id = ?
            ";
            $stmt1 = $conn->prepare($sql1);
            $stmt1->bind_param('ssi', $laboratory_tca, $hcw_name, $p_id);
            $stmt1->execute();
            $stmt1->close();
...
$laboratory_tca = $currentSettings['laboratory_tca'] ?? '';
...
<div class="container mt-5">
    ...
    <form method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" name="laboratory_tca"
                   value="<?= htmlspecialchars(date('Y-m-d', strtotime($laboratory_tca))) ?>" required>
        </div>
