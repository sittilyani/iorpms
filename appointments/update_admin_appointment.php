...
// Fetch admin record
$sql = "SELECT p_id, mat_id, admin_tca, hcw_name FROM patients WHERE p_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    $admin_tca_raw = trim($_POST['admin_tca']);
    $hcw_name            = trim($_POST['hcw_name']);

    // Normalize date (handles DATE or DATETIME)
    $admin_tca = date('Y-m-d', strtotime($admin_tca_raw));
    ...
    if (empty($errorMessage)) {
        try {
            ...
            $sql1 = "
                UPDATE patients
                SET admin_tca = ?, hcw_name = ?
                WHERE p_id = ?
            ";
            $stmt1 = $conn->prepare($sql1);
            $stmt1->bind_param('ssi', $admin_tca, $hcw_name, $p_id);
            $stmt1->execute();
            $stmt1->close();
...
$admin_tca = $currentSettings['admin_tca'] ?? '';
...
<head>
    <meta charset="UTF-8">
    <title>Update Admin Appointment</title>
    <link href="../assets/css/bootstrap.css" rel="stylesheet">
</head>
<body>

<div class="container mt-5">
    <h3 class="mb-4">Update Admin Appointment</h3>
    ...
    <form method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" name="admin_tca"
                   value="<?= htmlspecialchars(date('Y-m-d', strtotime($admin_tca))) ?>" required>
        </div>
        ...
        <a href="admin_appointments.php" class="btn btn-secondary">Back</a>
    </form>
</div>
