...
try {
    ...
    $sql = "
        SELECT
            p_id, comp_date, mat_id , clientName, hcw_name, nursing_tca,

            COALESCE(NULLIF(hcw_name, ''), 'no previous_visit') AS lab_notes,

            /* Derived appointment status */
            CASE
                WHEN DATE(nursing_tca) = CURDATE() + INTERVAL 1 DAY
                THEN 'scheduled'
                ELSE 'unscheduled'
            END AS appointment_status

        FROM patients
        WHERE nursing_tca >= CONCAT(?, ' 00:00:00')
          AND nursing_tca <= CONCAT(?, ' 23:59:59')

        ORDER BY nursing_tca ASC
    ";
...
<div class="header">
    <div class="container">
        <h1 class="display-5 fw-bold">Nursing Appointment Schedule</h1>
            ...
            <?php else: ?>
                <?php foreach ($appointments as $app): ?>
                    <tr>
                        ...
                        <td><?= formatDate($app['nursing_tca']) ?></td>
                        <td>
                        ...
                        <td>
                            <a href="update_nursing_appointment.php?p_id=<?= $app['p_id'] ?>"
