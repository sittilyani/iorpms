...
try {
    ...
    $sql = "
        SELECT
            p_id, comp_date, mat_id , clientName, hcw_name, laboratory_tca,

            COALESCE(NULLIF(hcw_name, ''), 'no previous_visit') AS lab_notes,

            /* Derived appointment status */
            CASE
                WHEN DATE(laboratory_tca) = CURDATE() + INTERVAL 1 DAY
                THEN 'scheduled'
                ELSE 'unscheduled'
            END AS appointment_status

        FROM patients
        WHERE laboratory_tca >= CONCAT(?, ' 00:00:00')
          AND laboratory_tca <= CONCAT(?, ' 23:59:59')

        ORDER BY laboratory_tca ASC
    ";
            ...
            <?php else: ?>
                <?php foreach ($appointments as $app): ?>
                    <tr>
                        ...
                        <td><?= formatDate($app['laboratory_tca']) ?></td>
                        <td>
                        ...
                        <td>
                            <a href="update_laboratory_appointment.php?p_id=<?= $app['p_id'] ?>"
