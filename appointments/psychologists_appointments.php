...
try {
    ...
    $sql = "
        SELECT
            p_id, comp_date, mat_id , clientName, hcw_name, psycho_social_tca,

            COALESCE(NULLIF(hcw_name, ''), 'no previous_visit') AS lab_notes,

            /* Derived appointment status */
            CASE
                WHEN DATE(psycho_social_tca) = CURDATE() + INTERVAL 1 DAY
                THEN 'scheduled'
                ELSE 'unscheduled'
            END AS appointment_status

        FROM patients
        WHERE psycho_social_tca >= CONCAT(?, ' 00:00:00')
          AND psycho_social_tca <= CONCAT(?, ' 23:59:59')

        ORDER BY psycho_social_tca ASC
    ";
...
<head>
    <meta charset="UTF-8">
    <title>Psychologist Appointment Schedule</title>
...
<div class="header">
    <div class="container">
        <h1 class="display-5 fw-bold">Psychologists Appointment Schedule</h1>
            ...
            <?php else: ?>
                <?php foreach ($appointments as $app): ?>
                    <tr>
                        ...
                        <td><?= formatDate($app['psycho_social_tca']) ?></td>
                        <td>
                        ...
                        <td>
                            <a href="update_psychologist_appointment.php?p_id=<?= $app['p_id'] ?>"
