...
try {
    ...
    $sql = "
        SELECT
            p_id, comp_date, mat_id , clientName, hcw_name, admin_tca,

            COALESCE(NULLIF(hcw_name, ''), 'no previous_visit') AS lab_notes,

            /* Derived appointment status */
            CASE
                WHEN DATE(admin_tca) = CURDATE() + INTERVAL 1 DAY
                THEN 'scheduled'
                ELSE 'unscheduled'
            END AS appointment_status

        FROM patients
        WHERE admin_tca >= CONCAT(?, ' 00:00:00')
          AND admin_tca <= CONCAT(?, ' 23:59:59')

        ORDER BY admin_tca ASC
    ";
...
<head>
    <meta charset="UTF-8">
    <title>Admin Appointment Schedule</title>
...
<div class="header">
    <div class="container">
        <h1 class="display-5 fw-bold">Admin Appointment Schedule</h1>
            ...
            <?php else: ?>
                <?php foreach ($appointments as $app): ?>
                    <tr>
                        ...
                        <td><?= formatDate($app['admin_tca']) ?></td>
                        <td>
                        ...
                        <td>
                            <a href="update_admin_appointment.php?p_id=<?= $app['p_id'] ?>"
