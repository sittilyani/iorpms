...
<body>
    ...
    $sql = "SELECT * FROM patients
            ...
            AND current_status IN ('active', 'defaulted') LIMIT 10";
    $sql = "
    SELECT
        patients.*,
        (SELECT DATE_FORMAT(MAX(dispDate), '%Y-%m-%d') FROM pharmacy WHERE pharmacy.mat_id = patients.mat_id) AS dispDate
    FROM
        patients
    WHERE
        (mat_id LIKE '%$search%'
        ...
        AND current_status IN ('defaulted', 'active')
";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                   ...
                   <td>" . (!empty($row['compDate']) ? htmlspecialchars($row['compDate']) :  "No visit Date") . "</td>
                    <td>
