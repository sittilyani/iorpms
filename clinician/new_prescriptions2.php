...
$stmt = $conn->prepare("SELECT COUNT(*) as count FROM other_prescriptions");
$stmt->execute();
$result = $stmt->get_result();
$row = $result->fetch_assoc();
$count = $row['count'] + 1;
$prescription_id = "PRESCR-" . str_pad($count, 8, '0', STR_PAD_LEFT);
$stmt->close();

// Fetch drug names for the dropdown
$drugs = [];
$result = $conn->query("SELECT drug_name FROM drugs ORDER BY drug_name ASC");
