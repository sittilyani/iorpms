...
<body>
    ...
    $sql = "SELECT * FROM viral_load
            WHERE (mat_id LIKE '%$search%' OR clientName LIKE '%$search%' OR dob LIKE '%$search%' OR sex LIKE '%$search%' OR hiv_status LIKE '%$search%' OR regimen_type LIKE '%$search%' OR results LIKE '%$search%')
            ";
    ...
    if ($result->num_rows > 0) {
        echo "<table border='1'>
               ...
               while ($row = $result->fetch_assoc()) {
                $maskedName = str_repeat('*', strlen($row['clientName'])); // Mask the client's name with asterisks
                echo "<tr>
                        <td>" . $row['vl_id'] . "</td>
                        <td>" . $row['mat_id'] . "</td>
                        <td>" . $maskedName . "</td> <!-- Masked name -->
                        <td>" . $row['dob'] . "</td>
                        <td>" . $row['reg_date'] . "</td>
                        <td>" . $row['sex'] . "</td>
                        <td>" . $row['hiv_status'] . "</td>
                        <td>" . $row['art_regimen'] . "</td>
                        <td>" . $row['regimen_type'] . "</td>
                        <td>" . $row['last_vlDate'] . "</td>
                        <td>" . $row['results'] . "</td>
                        <td>" . $row['next_appointment'] . "</td>
                        <td>" . $row['comp_date'] . "</td>
                        <td>
                            <a href='../clinician/view.php?mat_id=" . $row['mat_id'] . "'>View</a> &#124;
                            <a href='../clinician/update.php?mat_id=" . $row['mat_id'] . "'>Update</a> &#124;
                            <a href='delete.php?mat_id=" . $row['mat_id'] . "'>Delete</a>
                        </td>
                    </tr>";
            }
