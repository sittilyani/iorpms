...
$query = "SELECT * FROM statushistory";
if (!empty($dateFrom) && !empty($dateTo)) {
    $query .= " WHERE status_change_date BETWEEN '$dateFrom' AND DATE_ADD('$dateTo', INTERVAL 1 DAY)";
}

$query .= " ORDER BY status_change_date DESC";
...
echo "<h4 style='color: red; margin-bottom: 10px; font-family: Times New Roman;'>Status Change by Date</h4>";
...
if ($rowCount > 0) {
        echo "<div style='text-align: left; margin-bottom: 20px;'>
                        Total status updates between " . date('d-m-Y', strtotime($dateFrom)) . " and " . date('d-m-Y', strtotime($dateTo)) . " is: <span style='color:red; font-weight: bold;'> $rowCount  </span>
                    </div>";
} else {
        echo "<div style='text-align: left; font-weight: bold; color: red; margin-bottom: 20px;'>
                        No status updates happened on " . date('d-m-Y', strtotime($dateFrom)) . " and " . date('d-m-Y', strtotime($dateTo)) . ".
                    </div>";
}
...
echo "<tr>
                ...
                <th style='background-color: #0A1172; color: white;'>Previous Status</th>
                <th style='background-color: #0A1172; color: white;'>Previous Status Date</th>
                <th style='background-color: #0A1172; color: white;'>New Status</th>
                <th style='background-color: #0A1172; color: white;'>New Status Date</th>
        </tr>";
...
while ($row = $result->fetch_assoc()) {
        echo "<tr>";
                ...
                echo "<td>" . $row['status_change_date'] . "</td>";
        echo "</tr>";
}
echo "</table>";
...
<script>
    function exportToExcel() {
        ...
        link.href = uri;
        link.download = "view_status_change.xls"; // Use .xls for Excel 97-2003 compatibility
