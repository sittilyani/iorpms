...
<html>
<head>
    <title>defaulted patients</title>
...
<body>
    <h2 style="color: #2C3162; ">All defaulted Patients List</h2>
...
SELECT
    ...
    /*DATEDIFF(CURDATE(), (SELECT MAX(visitDate) FROM pharmacy WHERE pharmacy.mat_id = patients.mat_id)) AS days_defaulted*/
    COALESCE(DATEDIFF(CURDATE(), (SELECT MAX(visitDate) FROM pharmacy WHERE pharmacy.mat_id = patients.mat_id)), 0) AS days_defaulted /*if not visitDate found in pharmacy table, return, 0*/
FROM
    patients
WHERE
    (mat_id LIKE '%$search%'
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td style='color:red; font-weight: bold; text-align: center;'>" . ($row['days_defaulted'] !== null ? $row['days_defaulted'] : 'N/A') . "</td>
                    <td>
